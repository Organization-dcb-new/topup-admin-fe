...
export function Sidebar({ collapsed, mobileOpen, onToggleCollapse, onCloseMobile }: SidebarProps) {
  ...
  const NOC_ALLOWED = ['Dashboard', 'Transactions', 'Spending', 'Order', 'Overview']
    ...
    role === 'noc'
      ? sidebarMenus
          .map((section) => ({
            ...section,
            menus: section.menus
              .filter((menu) => NOC_ALLOWED.includes(menu.label))
              .map((menu) => ({
                ...menu,
                children: menu.children?.filter((child) => NOC_ALLOWED.includes(child.label)),
              })),
          }))
