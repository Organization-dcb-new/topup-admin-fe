...
async function copyTransactionId(value: string) {
  try {
...
export const paymentColumns: ColumnDef<Payment>[] = [
      ...
      return (
        <div className="flex max-w-[min(100%,18rem)] items-center gap-0.5">
          <Link
            ...
            title={`Buka detail transaksi — ${txId}`}
            aria-label={`Buka detail transaksi ${txId}`}
          >
  ...
  },
  {
    ...
    header: () => <span className="font-medium">Nomor bayar</span>,
    cell: ({ row }) => (
  ...
  },
  {
    ...
    header: () => <span className="font-medium">Aplikasi</span>,
    cell: ({ row }) => (
  ...
  },
  {
    ...
    header: () => <span className="font-medium">ID pesanan</span>,
    cell: ({ row }) => (
  ...
  },
  {
    accessorKey: 'amount',
    header: () => (
      <span className="block text-right font-medium">Nominal</span>
    ),
    cell: ({ row }) => (
  ...
  },
  {
    accessorKey: 'margin',
    header: () => (
      <span className="block text-right font-medium">Margin</span>
    ),
    cell: ({ row }) => (
  ...
  },
  {
    ...
    header: () => <span className="font-medium">Kanal</span>,
    cell: ({ row }) => (
  ...
  },
  {
    ...
    header: () => <span className="font-medium">Nomor VA</span>,
    cell: ({ row }) => (
  ...
  },
  {
    ...
    header: () => <span className="font-medium">Dibuat</span>,
    cell: ({ row }) => {
      const date = parseBackendDate(row.original.created_at)
      return (
        <span className="whitespace-nowrap text-sm tabular-nums text-muted-foreground">
          {date ? format(date, 'dd MMM yyyy, HH:mm:ss', { locale: id }) : '—'}
        </span>
      )
    },
  },
